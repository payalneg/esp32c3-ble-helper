...
/* Raw transmit (standard 11-bit ID). LispBM receives these via
 * (event-enable 'event-can-sid) — the channel for custom button commands. */
 ...
 * helper shows up by name next to the motor controllers.
 *
 ...
 * device (the BLE/WiFi MAC works) so two helpers can be told apart. */
