/*
    ...
    Big-endian byte buffer helpers used by the VESC packet protocol.
*/
...
/* Reads a null-terminated string from the buffer (VESC protocol uses
 * data + null terminator, no length prefix). Returns string length
 * (excluding terminator). buffer_len bounds the read. */
int buffer_get_string(const uint8_t *buffer, int buffer_len,
                      char *output, int max_len, int32_t *index);
