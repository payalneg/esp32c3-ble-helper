...
static char    s_fw_hw_name[32] = "VESC BLE Helper";
 ...
 * vTaskDelay(10 ms) added 0-10 ms to EVERY reassembled reply — a flat tax on
 * the request/reply round-trips that BLE<->CAN bridging is made of. */
...
{
    /* Standard 11-bit id frame — invisible to the VESC protocol decoder
     * (which is extended-id only) but delivered to LispBM scripts via
     * (event-enable 'event-can-sid). Used for custom button commands. */
    if (!s_init_done) return;
    if (len > 8) len = 8;

    twai_message_t tx_msg = {0};
    tx_msg.extd            = 0;
    tx_msg.identifier      = id & 0x7FFu;
    tx_msg.data_length_code = len;
    memcpy(tx_msg.data, data, len);
 ...
 *
 * Returns true when it handled the payload, i.e. the caller must NOT also feed
 * it to the packet handler (which parses COMM_FW_VERSION as a REPLY and would
 * read our own request as a garbage version). */
