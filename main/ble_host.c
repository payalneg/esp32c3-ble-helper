...
#define DEVICE_NAME "VESC-BLE-Helper"
                                      ...
                                      void *arg)
{
    ...
    ble_cfg_svc_gatts_register_cb(ctxt, arg);
}
...
static int gap_event_cb(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            ble_cfg_svc_on_connect(event->connect.conn_handle);
        } else {
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
         ...
         * peer doesn't get spammed with VESC RT-poll responses; the config
         * service tracks its own characteristics the same way. */
        ble_nus_on_subscribe(event->subscribe.attr_handle,
                             event->subscribe.cur_notify);
        ble_cfg_svc_on_subscribe(event->subscribe.attr_handle,
                                 event->subscribe.cur_notify);
        return 0;
    ...
    case BLE_GAP_EVENT_NOTIFY_TX:
        if (event->notify_tx.status != 0) {
            /* Aggregate congestion errors to one line/sec — see the P4
             * version of this file for the full war story. */
...
static void start_advertising(void)
{
    /* Adv data carries flags + the NUS service UUID-128 — VESC Tool filters
     * scans by this UUID. UUID-128 (18 B) + flags (3 B) leaves no room for
     * the device name in the 31-byte adv payload, so the name moves to the
     * scan response (the config GUI matches by name "VESC-BLE-Helper"). */
...
static void on_sync_cb(void)
{
    ...
    /* Dual-role: advertising (GUI/app peripheral) and the shared central
     * connect (button + cadence sensor) run concurrently. */
    ble_central_mgr_on_sync(s_own_addr_type);
}
...
esp_err_t ble_host_init(void)
{
    ...
    /* NimBLE logs every GATT notify at INFO — with the status stream plus
     * NUS traffic that's pure spam. Keep the stack's WARN/ERROR. */
    esp_log_level_set("NimBLE", ESP_LOG_WARN);

    /* Native controller — nimble_port_init brings it up on the C3. */
    ...
    /* Security for the HID button: bond with Just Works (no IO), keep the
     * keys in NVS so the button reconnects encrypted after reboot. */
    ...
    /* Standard GAP / GATT services must init before count_cfg/add_svcs. */
    ble_svc_gap_init();
    ble_svc_gatt_init();

    ble_central_mgr_init();
    ble_cadence_client_init();
    ble_hid_client_init();

    const struct ble_gatt_svc_def *nus_svcs = ble_nus_get_svcs();
    const struct ble_gatt_svc_def *cfg_svcs = ble_cfg_svc_get_svcs();
    int rc = ble_gatts_count_cfg(nus_svcs);
    if (rc != 0) {
        ESP_LOGE(TAG, "ble_gatts_count_cfg (nus) rc=%d", rc);
        return ESP_FAIL;
    }
    rc = ble_gatts_count_cfg(cfg_svcs);
    if (rc != 0) {
        ESP_LOGE(TAG, "ble_gatts_count_cfg (cfg) rc=%d", rc);
        return ESP_FAIL;
    }
    ...
    rc = ble_gatts_add_svcs(cfg_svcs);
    if (rc != 0) {
        ESP_LOGE(TAG, "ble_gatts_add_svcs (cfg) rc=%d", rc);
        return ESP_FAIL;
    }
