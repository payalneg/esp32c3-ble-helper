...
{
    if (!s_inited || !s_bound) return false;
...
{
    if (!s_bound || s_conn_handle != BLE_HS_CONN_HANDLE_NONE) return false;
    if (!addr_equal(&desc->peer_id_addr, &s_bound_addr) &&
        !addr_equal(&desc->peer_ota_addr, &s_bound_addr)) {
        return false;
    }
    s_conn_handle = desc->conn_handle;
    ESP_LOGI(TAG, "sensor connected, conn=%u", (unsigned)s_conn_handle);
...
void ble_cadence_on_disconnect(uint16_t conn_handle, int reason)
{
    ...
    ESP_LOGI(TAG, "sensor disconnect, reason=%d", reason);
    reset_link_handles();
    /* mgr re-arms the connect; the sensor will re-advertise when it wakes. */
}
...
void ble_cadence_on_notify(struct ble_gap_event *event)
{
    if (event->notify_rx.conn_handle != s_conn_handle ||
        s_rpm_val_handle == 0 ||
        event->notify_rx.attr_handle != s_rpm_val_handle) {
        return;
    }
    uint8_t b[2] = {0, 0};
    if (OS_MBUF_PKTLEN(event->notify_rx.om) >= 2 &&
        os_mbuf_copydata(event->notify_rx.om, 0, 2, b) == 0) {
        int16_t centi = (int16_t)((uint16_t)b[0] | ((uint16_t)b[1] << 8));
